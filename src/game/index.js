...
export const Game = (props) => {
  ...
  const handleGameTurn = (row, column) => async (e) => {
    ...
    if (props.connectedToPeer) {
      props.freezeTurns();
      await getPeer().connectedRTC.send({
        turnNumber,
        row,
        column,
        turn,
  ...
  useEffect(() => {
    ...
    if (connectedToPeer) {
      if (global.peer) {
        global.peer.connectedRTC && global.peer.connectedRTC.send({ action: "setGameState", size, successCriteria });
      } else {
        getPeer().connectedRTC && getPeer().connectedRTC.send({ action: "setGameState", size, successCriteria });
      }
    }
  }, [connectedToPeer]);
  ...
  useEffect(() => {
    ...
    if (row !== undefined && column !== undefined) {
      ...
      if (_isWinning) {
        ...
        getPeer().connectedRTC && getPeer().connectedRTC.send({ action: "gameOver", turn });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", textAlign: "center", height: "50vh" }}>
      <div className="game">
        {sizeArray.map((value, row) => (
          <div className="row" key={row + " row"}>
            {sizeArray.map((value, column) => {
              return (
                <div className="element" key={column + " column"} onClick={handleGameTurn(row, column)}>
                  {getState(row, column)}
                </div>
              );
            })}
